...
export default async function handler(req, res) {
  ...
  try {
    ...
    const orderData = body.order || body.data || body.payload || body;
    ...
    let rawItems = orderData.items || orderData.products || orderData.cart || orderData.order_items || [];
    ...
    const normalizedItems = rawItems.map((item, index) => ({
      ...
      name: item.name || item.title || item.product_name || item.item_name || 'Item Goomer',
      quantity: item.quantity || item.amount || item.qty || 1,
      obs: item.obs || item.notes || item.observation || (item.options ? item.options.map(o => o.name).join(', ') : ''),
      sector: item.category_name || item.sector || item.category || 'Cozinha'
    }));

    if (normalizedItems.length === 0) {
      const fallbackName = orderData.name || orderData.title || orderData.product || 'Água sem gás';
      normalizedItems.push({
        id: '1',
        name: fallbackName,
        quantity: orderData.quantity || 1,
        obs: orderData.obs || order.notes || '',
        sector: 'Cozinha'
      });
    }

    const goomerOrderIdStr = orderData.code || orderData.display_id || orderData.id || Math.floor(10000 + Math.random() * 90000);
    const tableOrClientName = orderData.table ? `Mesa ${orderData.table}` : (orderData.customer?.name || orderData.client_name || orderData.table_number ? `Mesa ${orderData.table_number}` : 'Mesa Balcão');
    ...
    const newKdsOrder = {
      ...
      ifood_id: null,
      order_type: orderData.delivery_type === 'DELIVERY' ? 'Delivery' : (orderData.table || orderData.table_number ? 'Mesa' : 'Balcão'),
      ...
      total_price: orderData.total || orderData.total_price || orderData.price || 0,
